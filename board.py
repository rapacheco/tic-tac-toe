...
Authors: Scott Rixner and Joe Warren - Rice University
Part of the course Principles of Computing by Coursera
Revised by Rafael Pacheco Ribeiro
"""
...
class TTTBoard:
    ...
    def __init__(self, dim, reverse = False, board = None):
        ...
        if board == None:
            # Create empty board
            self._board = [[consts["EMPTY"] for dummycol in range(dim)]
    ...
    def get_empty_squares(self):
        ...
        for row in range(self._dim):
            for col in range(self._dim):
                if self._board[row][col] == consts["EMPTY"]:
    ...
    def check_win(self):
        ...
        # check all lines
        for line in lines:
            if len(set(line)) == 1 and line[0] != consts["EMPTY"]:
                if self._reverse:
...
def switch_player(player):
    ...
    if player == consts["PLAYERX"]:
        return consts["PLAYERO"]
    else:
        return consts["PLAYERX"]
...
def play_game(mc_move_function, ntrials, reverse = False):
    ...
    # Print winner
    if winner == consts["PLAYERX"]:
        print("X wins!")
    elif winner == consts["PLAYERO"]:
        print("O wins!")
    elif winner == consts["DRAW"]:
        print("Tie!")
    else:
